// cmd/simulators/sim-mass-user-send-delay/main.go
//
// Like sim-mass-user-send, but throttles SUBMISSION: 10 goroutines launched per
// second (it does not gate on finalization between batches). Post-P1.2 hybrid model:
// senders are hybrid accounts pinned by 32-byte SEEDs; addresses are hash-derived ids.
//
// Flags/env:
//
//	--seeds   CSV string or path to .csv of 32-byte (64-hex) sender auth seeds
//	USER_HEX  recipient 32-byte account-id (hex)
//	VALIDATOR_URL_LIST  comma-separated validator base URLs (uses index 2 if present)
package main
...
func main() {
	...
	urlList := splitCSV(getenv("VALIDATOR_URL_LIST", ""))
	if len(urlList) == 0 {
		fmt.Fprintln(os.Stderr, "ERROR: VALIDATOR_URL_LIST is empty")
		os.Exit(1)
	}
	// Preserve the original behaviour of targeting the 3rd validator when available.
	base := urlList[0]
	if len(urlList) > 2 {
		base = urlList[2]
	}
	c := simkit.NewClient(base)
